...
static int handle_dismiss(sd_bus_message *msg, void *data,
	...
	while (true) {
		...
		} else if (strcmp(key, "no-history") == 0) {
			ret = sd_bus_message_read(msg, "v", "b", &no_history);
	...
	bool add_to_hisotry = !no_history;
	...
	wl_list_for_each(notif, &state->notifications, link) {
		if (notif->id == id || id == 0) {
			...
			if (group) {
				close_group_notifications(notif, MAKO_NOTIFICATION_CLOSE_DISMISSED, add_to_hisotry);
			} else if (all) {
				close_all_notifications(state, MAKO_NOTIFICATION_CLOSE_DISMISSED, add_to_hisotry);
			} else {
				close_notification(notif, MAKO_NOTIFICATION_CLOSE_DISMISSED, add_to_hisotry);
			}
			set_dirty(surface);
			break;
		}
...
static const sd_bus_vtable service_vtable[] = {
	...
	SD_BUS_METHOD("Dismiss", "a{sv}", "", handle_dismiss, SD_BUS_VTABLE_UNPRIVILEGED),
